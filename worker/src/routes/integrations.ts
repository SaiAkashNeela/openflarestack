...
route.delete('/:id', async (c) => {
  ...
  if (integration?.type === 'discord') {
    await stopDiscordGateway(c.env, integration.id)
  }
...
async function startDiscordGateway(
  ...
  const id = env.DISCORD_GATEWAY.idFromName(integrationId)
  const room = env.DISCORD_GATEWAY.get(id)
  await room.fetch(new Request('https://do/start', {
    method: 'POST',
    body: JSON.stringify({
      integrationId,
      organizationId,
...
async function stopDiscordGateway(env: AppEnv['Bindings'], integrationId: string) {
  const id = env.DISCORD_GATEWAY.idFromName(integrationId)
  const room = env.DISCORD_GATEWAY.get(id)
  await room.fetch(new Request('https://do/stop', { method: 'POST' }))
}
