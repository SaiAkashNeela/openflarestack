...
export function GoogleSignInButton({ label }: { label: string }) {
  ...
  const handleClick = async () => {
    ...
    try {
      ...
      setPending(false);
      toast({
        title: error instanceof Error ? error.message : "Google sign-in failed",
        tone: "error",
      });
    }
  };
  ...
  return (
    <button
      ...
      disabled={pending}
      className="mt-3 w-full rounded-md border border-border bg-background px-4 py-2 text-sm font-medium text-foreground hover:bg-surface-hover disabled:cursor-not-allowed disabled:opacity-60"
    >
