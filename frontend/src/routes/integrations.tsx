...
import { Mail, MessageCircle, Globe, Plus, RotateCw, Trash2 } from "lucide-react";
...
const CATALOG: Template[] = [
  {
    type: "email",
    name: "Email",
    description: "Connect a support inbox",
    icon: Mail,
    meta: "Inbound routing · SMTP",
  },
  {
    type: "telegram",
    name: "Telegram",
...
const SETUP_FIELDS: Record<string, SetupField[]> = {
  ...
  openai_compatible: [
    ...
    { key: "autoReplyEnabled", label: "Auto replies", type: "checkbox" },
  ],
  webhook: [
          ...
          setupTemplate
            ? setupTemplate.type === "webhook"
              ...
              : setupTemplate.type === "email"
                ? "Route a support address into the inbox, then send replies from the backend."
          ...
          <div className="space-y-4">
            ...
            {setupTemplate.type === "email" && (
              <div className="space-y-2 rounded-md border border-border bg-background p-3 text-xs text-muted-foreground">
                <div className="font-medium text-foreground">Email routing setup</div>
                <ul className="list-disc space-y-1 pl-4">
                  <li>Point your mail provider or inbound gateway at this address.</li>
                  <li>Use the same address below so Flaredesk can match replies to a conversation.</li>
                  <li>Replies are sent from the backend using the sender details you configure below.</li>
                </ul>
              </div>
            )}
            ...
            {setupTemplate.type === "openai_compatible" && (
              <div className="space-y-2 rounded-md border border-border bg-background p-3 text-xs text-muted-foreground">
                <div className="font-medium text-foreground">OpenAI-compatible endpoint</div>
                <ul className="list-disc space-y-1 pl-4">
                  <li>Use a provider endpoint that already has upstream keys stored, or use an OpenAI-compatible base URL.</li>
                  <li>The model name should match what the upstream endpoint expects.</li>
                  <li>Auto replies stay off until you toggle them on below.</li>
                </ul>
              </div>
            )}
            {setupTemplate.type === "webhook" ? (
                  ...
                  if (field.type === "checkbox") {
                    return (
                      ...
                      >
                        ...
                        <input
                          checked={setupConfig[field.key] === "true"}
                          onChange={(e) =>
                            setSetupConfig((prev) => ({
                              ...prev,
...
function formatIntegrationMeta(
  ...
  if (type === "openai_compatible") {
    ...
    const autoReply = isTruthyConfigValue(config.autoReplyEnabled);
    const prefix = model
      ? `${model} · ${baseUrl || "OpenAI-compatible"}`
      : baseUrl || `Connected · ${formatDate(integration.created_at)}`;
    return `${prefix} · Auto replies ${autoReply ? "on" : "off"}`;
  }
...
function defaultConfig(type: string) {
  switch (type) {
    ...
    case "openai_compatible":
      return { baseUrl: "", apiKey: "", model: "", autoReplyEnabled: "false" };
    case "webhook":
