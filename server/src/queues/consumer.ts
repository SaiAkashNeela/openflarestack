...
import { maybeCreateAutoReply } from '../integrations/auto-reply'
import { createNotifications, getConversationNotificationRecipients } from '../lib/notifications'

export async function processJob(job: QueueJob, env: Env) {
  ...
  throw new Error(`Unsupported queue job type ${(job as { type?: string }).type ?? 'unknown'}`)
}
...
async function handleInbound(job: Extract<QueueJob, { type: 'inbound' }>, env: Env) {
  ...
  const existingMessage = existingConversation
    ? await env.DB.prepare(
        'SELECT id FROM messages WHERE conversation_id = ? AND external_id = ? AND organization_id = ?',
      )
        .bind(convId, incoming.externalId, organizationId)
        .first<{ id: string }>()
    : null
  ...
  if (!existingMessage) {
    ...
    const conv = await env.DB.prepare(
      directConversationId
        ? 'SELECT id, subject FROM conversations WHERE organization_id = ? AND id = ?'
        : 'SELECT id, subject FROM conversations WHERE organization_id = ? AND external_id = ?',
    )
    ...
    const recipients = await getConversationNotificationRecipients(env, organizationId, conv.id)
    if (recipients.length > 0) {
      await createNotifications(env, recipients, {
        organizationId,
        actorUserId: null,
        type: 'conversation.message',
        title: `New message from ${incoming.customerName}`,
        body: conv.subject ?? incoming.text.slice(0, 120),
  ...
  await maybeCreateAutoReply(env, organizationId, convId)
}
...
async function handleWebhookDelivery(job: WebhookDeliveryJob, env: Env) {
  ...
  const headers = new Headers({
    ...
    'x-openflarestack-event': event.type,
    'x-openflarestack-delivery': delivery.id,
    'x-openflarestack-timestamp': timestamp,
  })
  ...
  if (config.secret) {
    headers.set('x-openflarestack-signature', await signWebhookDelivery(config.secret, body, timestamp))
  }
